...
prefix_ban_list = {'under': 1, 'with': 1, 'camera': 1, 'full': 1, 'black': 1, 'pink': 1, 'for': 1, 'body': 1,
                   'canon': 1, 'ef': 1, 'top': 1, 'w': 1, 'led': 1, 'tv': 1, 'kit': 1, 'lens': 1}
special_model = {'Canon': ['xt', 'xti', 'xs', 'xsi', 'm'],
...
def collecting_models(page_title, model_exist, model_list):
    ...
    for i in range(0, len(temp) - 1):
        ...
        if test_valid_prefix(temp[i]) and temp[i].lower() not in prefix_ban_list and test_valid_postfix(temp[i + 1]):
